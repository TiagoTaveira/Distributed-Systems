...
//import org.komparator.security.handler.AuthenticationHandler;
...
public class SupplierPortImpl implements SupplierPortType {
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		...
		//listProducts -> match description -> add to new List -> return List
		// elem.getDescription().toLowerCase().contains(descText.toLowerCase())

		List<ProductView> products = listProducts(); //lista de todos os produtos
		List<ProductView> productsFound = new ArrayList<ProductView>(); //lista de produts a devolver
			

		for (ProductView elem : products) {
			if(elem.getDesc().contains(descText)) {
					productsFound.add(elem);
			}
		}
	...
	public String buyProduct(String productId, int quantity)
		...
		if (getProduct(productId) == null)
            		throwBadProductId("Product not exists");
		//check quantity
		if (quantity <= 0)
			throwBadQuantity("Quantity cannot be null or negative");
		...
		if (pQuantity < quantity)
			throwInsufficientQuantity("Insufficient quantity in stock!");

		String id ="";
		Supplier sup = Supplier.getInstance();

		try{
		id = sup.buyProduct(productId, quantity);
